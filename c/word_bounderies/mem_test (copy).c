...
int main()
{
	...
	/*char src3[] = "welcome!! *** more space **** dont say :SIGABRT";*/
	char *ptr = src1;
	...
	ptr = (char*)memmove(src1 +WORD, src1, 15);
	printf("%s\n\n", ptr);
	/* check with 2 different strings
	...
	strcpy(src1, src3);
	printf("(left overloop)\nThis is before:\n\t\t\t  %s\n",src1);		
	ptr = (char*)MemCpy(src1 ,src3, 15);
	printf("after MemCpy:\n%s\n\n", ptr);

	strcpy(src1, src2);
	printf("(left overloop)\nThis is before,  MemMove, memmove :\n%s\n",src1);		
	ptr = (char*)MemMove(src1, src3, 15);
	printf("%s\n", ptr);
	strcpy(src1, src2);
	ptr = (char*)memmove(src1, src1 +WORD, 15);
	printf("%s\n\n", ptr);*/
