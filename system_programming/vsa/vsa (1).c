...
void Largest(vsa_t *vsa_o)
{
	...
	while (current_val != 0x90909090)
	{
		...
		/*this while loop iterate until the next negative block_size value... not so readable...*/
		while (current_val != 0x90909090)	/* what about while (current_val > 0) instead? */
		{
...
{
	ssize_t current_val = 0;
	ssize_t ssize = (ssize_t)size; /*change name to more meanningfull such as demanded_usr_size*/
	...
	/*more apropriate name is curr_available_size, real_size..*/
	while (current_val != 0x90909090 && current_val > ssize)
	{
		current_val = (current_val > 0) ? current_val : -current_val; /*liked that line!!*/
		run += current_val + BLOCK; /*run - change to more informative name*/
